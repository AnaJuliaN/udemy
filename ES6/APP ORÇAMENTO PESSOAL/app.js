...
class Bd {
  ...
  pesquisar(despesa) {
    ...
    if(despesa.descricao != ''){
      console.log('entrou na descricao')
      despesasFiltradas = despesasFiltradas.filter(d => d.descricao === despesa.descricao)
    }

    if(despesa.valor != ''){
      console.log('entrou no valor')
...
function carregaListaDespesas(despesas = Array(), filtro = false) {
  ...
  //selecionando o elemente tbody da tabela
  ...
  despesas.forEach(function(d) {
    ...
    //acessando o atributo id do obj despesa e associando o id como valor do botão
    btn.id = `id_despesa_${d.id}`  
    btn.onclick = function () {
      //remover a despesa
      //substituindo o id_despesa_1... por apenas o id
      let id = this.id.replace('id_despesa_', '')
      //alert(id)
      bd.remover(id)
      //após remover atualizar a pagina
...
function pesquisarDespesa() {
  ...
  //para não repetir o mesmo código duas vezes
